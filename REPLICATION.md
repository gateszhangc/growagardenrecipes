...
This document explains how the project replicates the `https://growagarden-recipes.com/` homepage and validates visual parity. Follow these steps whenever you need to resync the clone or rerun the automated checks.
...
4. Download static assets (CSS, JS files):
   ```bash
   node scripts/downloadAssets.js
   ```
   This downloads all referenced assets from the original site to `public/assets/`.
